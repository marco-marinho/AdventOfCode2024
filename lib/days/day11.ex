defmodule AOC2024.Day11 do
  ...
  def blink_fast(stones) do
    Enum.reduce(stones, Map.new(), fn {stone, count}, acc ->
      ...
      case stone do
        0 -> Map.update(acc, 1, count, &(&1 + count))
        ...
        x -> Map.update(acc, x * 2024, count, &(&1 + count))
      end
    end)
  end

  def solve(input, steps) do
    stones = input |> hd |> String.split(" ") |> Enum.map(&String.to_integer/1)
    |> Enum.reduce(%{}, fn stone, acc ->
      Map.put(acc, stone, 1)
    end)
    final_stones = Enum.reduce(1..steps, stones, fn _, acc ->
      blink_fast(acc)
    end)
