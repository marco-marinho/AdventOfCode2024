defmodule AOC2024.Day25 do
  ...
  def part1(input) do
    ...
    keys =
      Map.get(groups, false)
      |> Enum.map(fn block -> Enum.map(block, fn line -> String.graphemes(line) end) end)
    ...
    keys_count =
      Enum.map(keys, fn block ->
        Enum.reduce(block, {0, 0, 0, 0, 0}, fn line, acc -> count_line(line) |> sum_lines(acc) end)
      end)

    locks_count =
      Enum.map(locks, fn block ->
        Enum.reduce(block, {0, 0, 0, 0, 0}, fn line, acc -> count_line(line) |> sum_lines(acc) end)
      end)
