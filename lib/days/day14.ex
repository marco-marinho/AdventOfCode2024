defmodule AOC2024.Day14 do
  ...
  def check_tree(nodes) do
    ...
    |> Enum.map(fn {_, l} ->
      ...
      end) |> Enum.map(fn {max, _, _} -> max end) |> Enum.max()
  end

  def find_tree(positions, {w, h}, step) do
    new_pos = Enum.map(positions, fn {pos, vel} ->
  ...
  def parse(input) do
    Enum.map(input, fn line ->
      ...
      pos = String.replace(x, "p=", "") |> String.split(",") |> Enum.map(&String.to_integer/1) |> List.to_tuple()
      vel = String.replace(y, "v=", "") |> String.split(",") |> Enum.map(&String.to_integer/1) |> List.to_tuple()
      {pos, vel}
    end)
  end
  ...
  def part1(input) do
    ...
    final_position = Enum.map(positions, fn {pos, vel} ->
    ...
    end) |> Tuple.to_list |> Enum.reduce(1, fn x, acc -> x * acc end)
