defmodule AOC2024.Day24 do
  ...
  def resolve(inputs, connections) do
    {inputs, connections} = Enum.reduce(connections, {inputs, []}, fn {a, b, op, output}, {in_acc, con_acc} ->
  ...
  def part1(input) do
    ...
    resolve(inputs, connections) |> Map.filter(fn {k, _} -> String.contains?(k, "z") end)
    |> Enum.to_list()
    |> Enum.sort()
